...
class Zillow:
    ...
    def parseAllDataSections(self, soup=None):
        ...
        if listing_data['price'] > self.current_price:
            self.current_price = listing_data['price']
        if self.current_price < self.previous_price - 1:
            return False
        self.async_count = 0
    ...
    def async_scrape_url(self, url):  ## Function to process requests concurrently
        ...
        for _ in range(NUM_RETRIES):
            try:
                ...
                print("Connection Error line 137")
                response = ''
        if response.status_code != 200: ## parse data if 200 status code (successful response)
            ...
            if soup is not None:
                ...
                print("soup is none line 149")
            


    def sub(self, executor, listing_links):
       
        futures = executor.map(self.async_scrape_url, listing_links)
    ...
    def stop_process_pool(self, executor):
                for pid, process in executor._processes.items():
                    process.terminate()
                executor.shutdown()
    '''
    made this to test if it would stop the process pool however run Pebble did not work
    '''    
    def runPebble(self,listing_links):
        pool = pebble.ProcessPool(max_workers=20)
        with pool:
      
            future = pool.map(self.async_scrape_url, listing_links, timeout=15)
    ...
    '''
    current runAsync function waits till all threads are done before moving on to the next page
    
    '''
    def runAsyncByLoop(self,listing_links):
        with futures.ThreadPoolExecutor(max_workers=20) as executor: 
            the_futures = [executor.submit(self.async_scrape_url, link) for link in listing_links]
    ...
    def runAsync(self,listing_links):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
            ...
            futures = executor.map(self.async_scrape_url, listing_links)#, timeout=30)
            list(futures)
    ...
    #                 for future in concurrent.futures.as_completed(executor.map(self.async_scrape_url, listing_links, timeout=30),timeout=30):
    #                     print(future.result(timeout=10))
    #             except concurrent.futures._base.TimeoutError:
    #                 print("This took to long...")
    #                 self.stop_process_pool(executor)
